...
typedef std::pair<int, OpenAIModel> pair_model;
typedef std::vector<pair_model> vec_models;
...
void print_models(vec_models &models)
{
    ...
    std::sort(models.begin(), models.end(), [](const pair_model &a, const pair_model &b) {
        if (a.first != b.first) {
